...
resourcename="shoes"  # lower case, only
resourcegroup="shoes_rg"
...
# git clone https://github.com/apilogicserver/shoes.git
# cd shoes
...
az group create --name shoes_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group shoes_rg --sku S1 --is-linux
az container create --resource-group shoes_rg --name shoes --image apilogicserver/shoes:latest --dns-name-label shoes --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//shoes.westus.azurecontainer.io:5656'
